...
// global scene variable which will hold the game view
							...
							LPARAM	lParam)			// Additional Message Information
{
	switch (uMsg)									// Check For Windows Messages
	{
		...
		case WM_SIZE:								// Resize The OpenGL Window
		{
			...
			//scene->resizeScene(GetSystemMetrics(SM_CXSCREEN),HIWORD(lParam));
					...
					int			nCmdShow)			// Window Show State
{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
                                            ...
                                            // If There Are No Messages
		{
			...
			if ((active && !scene->drawScene()) || keys[VK_ESCAPE])	// Active?  Was There A Quit Received?
			{
			...
			if (keys[VK_F1])						// Is F1 Being Pressed?
			{
				...
				fullscreen=!fullscreen;				// Toggle Full screen / Windowed Mode
    ...
    // free memory when the game is closed
	delete scene;
