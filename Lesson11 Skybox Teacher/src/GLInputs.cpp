...
void GLInputs::mouseMove(GLModel* mdl, double x, double y)
{
      ...
      if(mouse_Rotate)
      {
        ...
        // if not using skybox then uncomment these lines
         // prev_MouseX =x;                      // update previous mouse x
          // prev_MouseY =y;                      // update previous mouse y
      }
}
...
void GLInputs::keyPressed(GLSounds* SD)
{
    switch(wParam)
    {
    case VK_SPACE:
        SD->playSound("sounds/explosion.wav");
        break;

    case VK_TAB:
        SD->pauseSound("sounds/ophelia.mp3");
        break;
...
void GLInputs::keyPressed(GLSkyBox* sky)
{
    switch(wParam)
    {
    case VK_LEFT:
...
void GLInputs::mouseMove(GLSkyBox* sky, double x, double y){
    sky->Rot.y += (x-prev_MouseX) / 3.0;
    sky->Rot.x += (prev_MouseY-y) / 3.0;
    prev_MouseX = x;
    prev_MouseY = y;
}
